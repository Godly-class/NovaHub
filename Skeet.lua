...
frame.Size = UDim2.new(0, 600, 0, 26)
frame.Position = UDim2.new(0, 10, 0, 10)
frame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
frame.BackgroundTransparency = 0.4
frame.BorderSizePixel = 2
frame.BorderColor3 = Color3.fromRGB(255, 100, 149)
frame.Active = true
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if fpsTimer >= 0.5 then
        ...
        currentFps = math.floor(sum / #fpsBuffer)
        fpsBuffer = {}
        fpsTimer = 0
        ...
        label.Text = string.format(
            "[ YourName | UID : %d | %d ping | %d fps | %s | %s ]",
            player.UserId,
            ping,
            currentFps,
            timeStr,
...
local SafeESP = {
    Enabled = false,
    bredFolder = workspace:WaitForChild("Map"):WaitForChild("Safes")
}

local function UpdateSafeESP()
    if not SafeESP.Enabled then 
        -- 清理所有 Highlight 和 Billboard
        for _, safe in ipairs(SafeESP.bredFolder:GetChildren()) do
            local hl = safe:FindFirstChild("SafeHighlight")
            if hl then hl:Destroy() end
            local bb = safe:FindFirstChild("SafeBillboard")
    ...
    for _, safe in ipairs(SafeESP.bredFolder:GetChildren()) do
        local mainPart = safe.PrimaryPart or safe:FindFirstChildOfClass("BasePart")
        ...
        -- Highlight
        local highlight = safe:FindFirstChild("SafeHighlight") or Instance.new("Highlight")
        highlight.Name = "SafeHighlight"
        highlight.Adornee = safe
        ...
        -- Billboard
        local billboard = safe:FindFirstChild("SafeBillboard") or Instance.new("BillboardGui")
        ...
        label.Size = UDim2.new(1, 0, 1, 0)
        label.BackgroundTransparency = 1
        label.Text = isBroken and "[BROKEN] " .. safe.Name or "[SAFE] " .. safe.Name
...
SafeSection:CreateToggle("Safe ESP", false, function(v)
    SafeESP.Enabled = v
end)

SafeSection:CreateToggle("Auto Hack Safe", false, function(v)
    AutoHackEnabled = v
end)

SafeSection:CreateButton("Hack Nearest Safe", function()
    local nearest = nil
    ...
    if hrp then
        for _, safe in ipairs(SafeESP.bredFolder:GetChildren()) do
            local part = safe.PrimaryPart or safe:FindFirstChildOfClass("BasePart")
            if part then
...
SafeSection:CreateToggle("Auto Pickup Money", false, function(v)
    if v then
        StartAutoPickup()
    else
...
SafeSection:CreateSlider("Pickup Distance", 10, 60, 25, " studs", function(v)
    Settings.PickupDistance = v
end)
-- ==================== AntiAim 完整邏輯 + UI ====================
...
local function updateHideHeadHook()
    if hideHeadEnabled then
        if not originalHook then
            originalHook = hookmetamethod(game, "__namecall", function(self, ...)
                local method = getnamecallmethod()
                if method == "FireServer" and tostring(self) == "MOVZREP" then
                    local fixedArgs = {{
                        {
                            Vector3.new(-5721.2001953125, -5, 971.5162353515625),
                            Vector3.new(-4181.38818359375, -6, 11.123311996459961),
                    ...
                    return originalHook(self, unpack(fixedArgs))
                end
                return originalHook(self, ...)
            end)
        end
...
local function loadInvisAnim()
    if InvisAnimTrack then InvisAnimTrack:Stop() end
    if humanoid then
        local success, track = pcall(function()
            return humanoid:LoadAnimation(InvisAnim)
        end)
        if success then
...
RunService.Heartbeat:Connect(function(dt)
    if not InvisEnabled then return end
    updateChar()
    if not character or not humanoid or not hrp or humanoid.Health <= 0 then return end
    ...
    hrp.CFrame = CFrame.new(hrp.Position) * CFrame.fromOrientation(0, cameraYaw, 0)
...
-- ==================== AntiAim UI (直接建立) ====================
local Tab4 = UI:CreateTab("AntiAim")

local Left = Tab4:CreateSection("Hide Head", "Left")
local Right = Tab4:CreateSection("Invisibility", "Right")

Left:CreateToggle("Hide Head", false, function(v)
    hideHeadEnabled = v
    updateHideHeadHook()
end)

Right:CreateToggle("Invisibility", false, function(v)
    if v then
        enableInvis()
    else
        disableInvis()
...
Right:CreateButton("Force Disable Invisibility", function()
    disableInvis()
end)

Right:CreateButton("Reload Invis Animation", function()
    loadInvisAnim()
end)